...
/// SHA256 digest of a single block of data.
#[inline(always)]
fn digest(mut data: [u8; 64], len: usize) -> [u32; 8] {
    debug_assert!(len <= 55);

    // add padding
    data[len] = 0x80;
    data[62] = (((len * 8) >> 8) & 0xff) as u8;
    data[63] = ((len * 8) & 0xff) as u8;
    ...
    unsafe { process(&mut state, &data, 64) };
...
unsafe fn process(state: &mut [u32; 8], mut data: &[u8], mut len: u32) {
